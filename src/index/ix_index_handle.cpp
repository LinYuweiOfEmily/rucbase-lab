...
bool IxNodeHandle::leaf_lookup(const char *key, Rid **value) {
    ...
    if (ix_compare(get_key(key_idx), key, file_hdr->col_types_, file_hdr->col_lens_) == 0 &&  key_idx != get_size()) {
...
page_id_t IxNodeHandle::internal_lookup(const char *key) {
    ...
    Rid* child_rid = get_rid(key_idx - 1);
...
void IxNodeHandle::insert_pairs(int pos, const char *key, const Rid *rid, int n) {
	...
	memmove(begin_key + n*file_hdr->col_tot_len_, begin_key, (get_size() - pos)*file_hdr->col_tot_len_);
	memcpy(begin_key, key, n*file_hdr->col_tot_len_);
	memmove(begin_rid +n, begin_rid, (get_size() - pos)*sizeof(Rid));
...
std::pair<IxNodeHandle *, bool> IxIndexHandle::find_leaf_page(const char *key, Operation operation,
    ...
    bool is_lock = false;
    if (root_latch_.try_lock()) {
        is_lock = false;
        root_latch_.unlock();
    } else {
        is_lock = true;
    }
    IxNodeHandle* ans_node = root;
    while (!ans_node->is_leaf_page()) {
        IxNodeHandle *parent = ans_node;
        ans_node = fetch_node(ans_node->internal_lookup(key)); 
        buffer_pool_manager_->unpin_page(parent->get_page_id(), false);
    }
    return std::make_pair(ans_node, is_lock);
}
...
bool IxIndexHandle::get_value(const char *key, std::vector<Rid> *result, Transaction *transaction) {
    ...
    auto[leaf_node_handle, is_lock] = find_leaf_page(key, Operation::FIND, transaction);
    Rid* rid = nullptr;
    buffer_pool_manager_->unpin_page(leaf_node_handle->get_page_id(), false);
    if (leaf_node_handle->leaf_lookup(key, &rid)) {
...
page_id_t IxIndexHandle::insert_entry(const char *key, const Rid &value, Transaction *transaction) {
    ...
    auto[leaf_handle, is_lock] = find_leaf_page(key, Operation::INSERT, transaction);

    int cur_size = leaf_handle->get_size();

    // 2. 在该叶子节点中插入键值对
    if (leaf_handle->insert(key, value) == cur_size) {
        buffer_pool_manager_->unpin_page(leaf_handle->get_page_id(), false);	
		return false;
        
    } else if (leaf_handle->get_size() == leaf_handle->get_max_size()) {
        // 3. 如果结点已满，分裂结点，并把新结点的相关信息插入父节点
        IxNodeHandle* new_node = split(leaf_handle);
        if(leaf_handle->get_page_no() == file_hdr_->last_leaf_)
			file_hdr_->last_leaf_ = new_node->get_page_no(); 
		insert_into_parent(leaf_handle, new_node->get_key(0), new_node, transaction);
		buffer_pool_manager_->unpin_page(new_node->get_page_id(), true);//unpin
    }
    
    // 提示：记得unpin page；若当前叶子节点是最右叶子节点，则需要更新file_hdr_.last_leaf；记得处理并发的上锁
    buffer_pool_manager_->unpin_page(leaf_handle->get_page_id(), false);//unpin
    return true;
}
...
bool IxIndexHandle::delete_entry(const char *key, Transaction *transaction) {
    ...
    // 1. 获取该键值对所在的叶子结点
    std::scoped_lock lock{root_latch_};
    auto[leaf, is_lock] = find_leaf_page(key, Operation::DELETE, transaction);
...
Iid IxIndexHandle::lower_bound(const char *key) {
    auto[node, is_lock] = find_leaf_page(key, Operation::FIND, nullptr);
